...
export default {
	...
	computed: {
		...
		currentIndex() {
			...
			//calculte currentIndex
			const index = tops.findIndex((top, idx) => {
				return scrollY >= top && scrollY < tops[idx + 1]
			})
			return index
		}
	},
	methods: {
		...
		_initTops() {
			...
			let top = 0
			tops.push(0)
			Array.from(this.$refs.foodul.children).forEach(li => {
				top += li.clientHeight
				tops.push(top)
			})
			this.tops = tops
